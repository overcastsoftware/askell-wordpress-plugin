...
class Main {
	...
	public function delete_customer_on_user_delete( int $user_id ) {
		...
		$deletion = $askell_askellapi->delete_customer_in_askell_by_user_id(
			$user_id
		);
		if ( false === $deletion ) {
	...
	public function push_customer_on_user_update( int $user_id ) {
		...
		return $askell_askellapi->push_customer( $user );
	}
	...
	public function save_customer_to_user( WP_User $user ) {
		...
		$customer = $askell_askellapi->pull_customer( $user );
	...
	public function save_customer_subscriptions_to_user( WP_User $user ) {
		...
		$subscriptions = $askell_askellapi->pull_customer_subscriptions(
			$user
		);
	...
	public function save_plans() {
		...
		$plans = $askell_askellapi->pull_plans();
		if ( false === $plans ) {
			return false;
		}
